...
const AdminEventos = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      // Cargar eventos con timestamp para evitar caché
      const eventosResponse = await api.get(`/eventos?t=${Date.now()}`);
      const eventosData = eventosResponse.data.data?.eventos || [];

      console.log("Datos de eventos recargados:", eventosData);
      ...
      const totalInscripciones = eventosData.reduce(
        (sum, e) => sum + (e.total_inscripciones || 0),
        0
      );
      const capacidadTotal = eventosData.reduce(
        (sum, e) => sum + parseInt(e.capacidad_max_eve || 0),
        0
      );
  ...
  const handleOpenModal = (eventoData = null) => {
    ...
    if (eventoData) {
      ...
      setEvento({
        ...
        hor_ini_eve: formatearHoraParaInput(
          eventoData.hora_inicio || eventoData.hor_ini_eve
        ),
        hor_fin_eve: formatearHoraParaInput(
          eventoData.hora_fin || eventoData.hor_fin_eve
        ),
        ...
        es_gratuito:
          eventoData.es_gratuito !== undefined ? eventoData.es_gratuito : true,
        precio: eventoData.precio || "",
        carreras_seleccionadas: eventoData.carreras
          ? eventoData.carreras.map((c) => c.id)
          : [],
        porcentaje_asistencia_aprobacion:
          eventoData.porcentaje_asistencia_aprobacion || 80,
      });
    } else {
  ...
  const handleSubmitModal = async () => {
    ...
    if (!evento.nom_eve.trim())
      nuevosErrores.nom_eve = "El nombre es obligatorio";
    ...
    if (
      ...
      evento.carreras_seleccionadas.length === 0
    ) {
      nuevosErrores.carreras_seleccionadas =
    ...
    try {
      // Preparar datos del evento con carreras incluidas
      const datosEvento = {
        ...
        porcentaje_asistencia_aprobacion: parseInt(
          evento.porcentaje_asistencia_aprobacion,
          10
        ),
      };
      ...
      if (evento.tipo_audiencia_eve === "CARRERA_ESPECIFICA") {
        datosEvento.carreras = evento.carreras_seleccionadas;
      } else if (evento.tipo_audiencia_eve === "TODAS_CARRERAS") {
        // Para "TODAS_CARRERAS", enviar todas las carreras disponibles
        datosEvento.carreras = carreras.map((carrera) => carrera.id_car);
      } else {
        // Para "PUBLICO_GENERAL", no enviar carreras
        datosEvento.carreras = [];
      }
      ...
      // Pequeño delay para asegurar que el backend procese todos los cambios
      setTimeout(async () => {
        await cargarDatos();
      }, 500);
      ...
      setSnackbar({
        ...
        message:
          ...
          `Error al ${isEditing ? "actualizar" : "crear"} el evento: ${
            error.message
          }`,
        severity: "error",
      });
    ...
    <Box sx={{ display: "flex", height: "100vh", bgcolor: "#f5f5f5" }}>
      ...
      >
        ...
        <Box sx={{ mb: 4 }}>
          <Typography
            variant="h4"
            sx={{ color: "#6d1313", fontWeight: "bold", mb: 1 }}
          >
                  ...
                  <CardContent sx={{ flexGrow: 1, p: 3, width: "400px" }}>
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      >
                        ...
                        <Typography variant="body2" color="text.secondary">
                          {formatearHora(
                            evento.hora_inicio || evento.hor_ini_eve
                          )}
                          {(evento.hora_fin || evento.hor_fin_eve) &&
                            ` - ${formatearHora(
                              evento.hora_fin || evento.hor_fin_eve
                            )}`}
                        </Typography>
                      ...
                      >
                        ...
                        />
                        <Typography variant="body2" color="text.secondary">
                          {evento.total_inscripciones || 0} /{" "}
                          {evento.capacidad_max_eve || "Sin límite"}
                        </Typography>
                      </Box>
                        ...
                        <Chip
                          label={
                            evento.tipo_audiencia_eve === "CARRERA_ESPECIFICA"
                              ...
                              ? "PÚBLICO GENERAL"
                              : evento.tipo_audiencia_eve.replace(/_/g, " ")
                          }
                          size="small"
                          variant="outlined"
                          sx={{ mb: 1 }}
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth error={!!errors.id_cat_eve}>
                      ...
                      {errors.id_cat_eve && (
                        <FormHelperText>{errors.id_cat_eve}</FormHelperText>
                      )}
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth error={!!errors.are_eve}>
                      ...
                      {errors.are_eve && (
                        <FormHelperText>{errors.are_eve}</FormHelperText>
                      )}
                    </FormControl>
                  </Grid>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth error={!!errors.ced_org_eve}>
                      ...
                      {errors.ced_org_eve && (
                        <FormHelperText>{errors.ced_org_eve}</FormHelperText>
                      )}
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item xs={12} md={6}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setEvento((prev) => ({
                          ...prev,
                          capacidad_max_eve: e.target.value,
                        }))
                      }
                      error={!!errors.capacidad_max_eve}
                      helperText={
                        errors.capacidad_max_eve || "Máximo 1000 personas"
                      }
                    ...
                    <FormControl fullWidth error={!!errors.tipo_audiencia_eve}>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setEvento((prev) => ({
                            ...prev,
                            tipo_audiencia_eve: e.target.value,
                          }))
                        }
                        ...
                        >
                          {carreras.map((carrera) => (
                            ...
                            >
                              <Checkbox
                                checked={
                                  evento.carreras_seleccionadas.indexOf(
                                    carrera.id_car
                                  ) > -1
                                }
                              />
                  ...
                  {!evento.es_gratuito && (
                    <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        helperText={
                          errors.precio ||
                          "Ingrese el precio del evento en dólares"
                        }
                        inputProps={{
                          min: 0.01,
                          max: 10000,
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      ...
                      {evento.porcentaje_asistencia_aprobacion}% de asistencia
                      para aprobar el evento y recibir su certificado.
                    </Alert>
                  </Grid>
                </Grid>
              </Grid>
