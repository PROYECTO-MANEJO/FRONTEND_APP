...
const AdminCursos = () => {
  ...
  const TIPOS_AUDIENCIA = [
    {
      value: "CARRERA_ESPECIFICA",
      label: "Carrera Específica",
      description: "Solo para estudiantes de carreras seleccionadas",
    },
    {
      value: "TODAS_CARRERAS",
      label: "Todas las Carreras",
      description: "Para estudiantes de todas las carreras de la universidad",
    },
    {
      value: "PUBLICO_GENERAL",
      label: "Público General",
      description: "Para estudiantes y usuarios externos",
    },
  ];
  ...
  const cargarDatos = async () => {
    try {
      ...
      const totalInscripciones = cursosData.reduce(
        (sum, c) => sum + (c.total_inscripciones || 0),
        0
      );
      const capacidadTotal = cursosData.reduce(
        (sum, c) => sum + parseInt(c.capacidad_max_cur || 0),
        0
      );
  ...
  const handleOpenModal = (cursoData = null) => {
    ...
    if (cursoData) {
      ...
      setCurso({
        ...
        carreras_seleccionadas: cursoData.carreras
          ? cursoData.carreras.map((c) => c.id)
          : [],
        requiere_verificacion_docs:
          cursoData.requiere_verificacion_docs !== undefined
            ? cursoData.requiere_verificacion_docs
            : true,
        es_gratuito:
          cursoData.es_gratuito !== undefined ? cursoData.es_gratuito : true,
        precio: cursoData.precio || "",
        porcentaje_asistencia_aprobacion:
          cursoData.porcentaje_asistencia_aprobacion || 80,
  ...
  const handleSubmitModal = async () => {
    ...
    if (
      ...
      curso.carreras_seleccionadas.length === 0
    ) {
    ...
    if (
      ...
      nuevosErrores.porcentaje_asistencia_aprobacion =
        "El porcentaje de asistencia mínimo es obligatorio";
    } else {
      const porcentajeAsistencia = parseFloat(
        curso.porcentaje_asistencia_aprobacion
      );
      if (
        isNaN(porcentajeAsistencia) ||
    ...
    if (!curso.nota_minima_aprobacion || curso.nota_minima_aprobacion === "") {
      nuevosErrores.nota_minima_aprobacion =
        "La nota mínima de aprobación es obligatoria";
    } else {
      const notaMinima = parseFloat(curso.nota_minima_aprobacion);
      if (isNaN(notaMinima) || notaMinima < 0 || notaMinima > 10) {
        nuevosErrores.nota_minima_aprobacion =
          "La nota mínima debe ser un número entre 0 y 10";
      }
    }
    ...
    try {
      ...
      const cursoData = {
        ...
        porcentaje_asistencia_aprobacion: parseFloat(
          curso.porcentaje_asistencia_aprobacion
        ),
        nota_minima_aprobacion: parseFloat(curso.nota_minima_aprobacion),
      };

      // Agregar carreras según el tipo de audiencia
      if (curso.tipo_audiencia_cur === "CARRERA_ESPECIFICA") {
        cursoData.carreras = curso.carreras_seleccionadas;
      } else if (curso.tipo_audiencia_cur === "TODAS_CARRERAS") {
        // Para "TODAS_CARRERAS", enviar todas las carreras disponibles
        cursoData.carreras = carreras.map((carrera) => carrera.id_car);
      } else {
        // Para "PUBLICO_GENERAL", no enviar carreras
        cursoData.carreras = [];
      }
      ...
      // Recargar datos con un pequeño delay para asegurar consistencia
      setTimeout(async () => {
        await cargarDatos();
      }, 500);
  ...
  const handleCerrarCurso = async (cursoId, nombreCurso) => {
    ...
    if (
      window.confirm(
        `¿Estás seguro de que quieres cerrar el curso "${nombreCurso}"? Esta acción generará certificados automáticamente para los participantes aprobados y no se puede deshacer.`
      )
    ) {
      try {
        ...
        const cursoActualizado = cursosActualizados.data.cursos.find(
          (c) => c.id_cur === cursoId
        );
  ...
  const obtenerEstadoCurso = (fechaInicio, fechaFin, estadoDB) => {
    ...
    if (estadoDB === "CERRADO") {
      console.log("✅ Curso cerrado en BD, retornando FINALIZADO para:", {
        estadoDB,
      });
      return "FINALIZADO";
    }
  ...
  const CursoEstadoChip = ({ curso }) => {
    const { estado: estadoDisplay, color: estadoColor } = useEstadoDisplay(
      curso,
      "curso"
    );
  ...
  return (
    <Box sx={{ display: "flex", height: "100vh", bgcolor: "#f5f5f5" }}>
      ...
      <Box
        sx={{ flexGrow: 1, overflow: "auto", p: 3, ...getMainContentStyle() }}
      >
        <Typography
          variant="h4"
          sx={{ mb: 1, fontWeight: "bold", color: "#6d1313" }}
        >
          Gestión de Cursos
        </Typography>
                ...
                >
                  <CardContent sx={{ flexGrow: 1, p: 2, width: "400px" }}>
                    ...
                    >
                      ...
                      <Avatar
                        sx={{ bgcolor: "#6d1313", width: 32, height: 32 }}
                      >
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      >
                        ...
                        />
                        <Typography variant="body2" color="text.secondary">
                          {curso.total_inscripciones || 0} /{" "}
                          {curso.capacidad_max_cur || "Sin límite"}
                        </Typography>
                      </Box>
                    ...
                    >
                      ...
                      {obtenerEstadoCurso(
                        ...
                        <IconButton
                          ...
                          title="Cerrar curso"
                          onClick={() =>
                            handleCerrarCurso(curso.id_cur, curso.nom_cur)
                          }
                        >
                          <Close />
                        </IconButton>
        ...
        >
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth error={!!errors.id_cat_cur}>
                      ...
                      {errors.id_cat_cur && (
                        <FormHelperText>{errors.id_cat_cur}</FormHelperText>
                      )}
                    </FormControl>
                  </Grid>
                      ...
                      >
                        {organizadores.map((org) => (
                          <MenuItem key={org.ced_org} value={org.ced_org}>
                            {org.nom_org1} {org.nom_org2} {org.ape_org1}{" "}
                            {org.ape_org2}
                          </MenuItem>
                        ))}
                      </Select>
                      {errors.ced_org_cur && (
                        <FormHelperText>{errors.ced_org_cur}</FormHelperText>
                      )}
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item xs={12} md={6}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setCurso((prev) => ({
                          ...prev,
                          capacidad_max_cur: e.target.value,
                        }))
                      }
                      error={!!errors.capacidad_max_cur}
                      helperText={
                        errors.capacidad_max_cur || "Máximo 1000 personas"
                      }
                ...
                <Grid container spacing={2}>
                    ...
                    <FormControl fullWidth error={!!errors.tipo_audiencia_cur}>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setCurso((prev) => ({
                            ...prev,
                            tipo_audiencia_cur: e.target.value,
                          }))
                        }
                  ...
                  {curso.tipo_audiencia_cur === "CARRERA_ESPECIFICA" && (
                    <Grid item xs={12}>
                      <FormControl
                        fullWidth
                        error={!!errors.carreras_seleccionadas}
                      >
                        ...
                        <Select
                          ...
                          onChange={(e) =>
                            setCurso((prev) => ({
                              ...prev,
                              carreras_seleccionadas: e.target.value,
                            }))
                          }
                          input={
                            <OutlinedInput label="Carreras Específicas *" />
                          }
                        ...
                        >
                          {carreras.map((carrera) => (
                            ...
                            >
                              <Checkbox
                                checked={
                                  curso.carreras_seleccionadas.indexOf(
                                    carrera.id_car
                                  ) > -1
                                }
                              />
                    ...
                    <Box sx={{ display: "flex", alignItems: "center" }}>
                      <Checkbox
                        ...
                        onChange={(e) =>
                          setCurso((prev) => ({
                            ...prev,
                            requiere_verificacion_docs: e.target.checked,
                          }))
                        }
                      />
                      <Typography>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center" }}>
                      <Checkbox
                        ...
                        onChange={(e) => {
                          ...
                          console.log(
                            "Cambiando requiere_carta_motivacion a:",
                            newValue
                          );
                          setCurso((prev) => ({
                            ...prev,
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12}>
                    <FormControl error={!!errors.es_gratuito}>
                      ...
                      {errors.es_gratuito && (
                        <FormHelperText>{errors.es_gratuito}</FormHelperText>
                      )}
                    </FormControl>
                  </Grid>
                  ...
                  {!curso.es_gratuito && (
                    <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        helperText={
                          errors.precio ||
                          "Ingrese el precio del curso en dólares"
                        }
                        inputProps={{
                          min: 0.01,
                          max: 10000,
              ...
              <Grid item xs={12}>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={6}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setCurso((prev) => ({
                          ...prev,
                          porcentaje_asistencia_aprobacion: e.target.value,
                        }))
                      }
                      error={!!errors.porcentaje_asistencia_aprobacion}
                      helperText={
                        errors.porcentaje_asistencia_aprobacion ||
                  ...
                  <Grid item xs={12} md={6}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setCurso((prev) => ({
                          ...prev,
                          nota_minima_aprobacion: e.target.value,
                        }))
                      }
                      error={!!errors.nota_minima_aprobacion}
                      helperText={
                        errors.nota_minima_aprobacion ||
                        "Nota mínima para aprobar el curso (0-10)"
                      }
                      inputProps={{
                        min: 0,
                        max: 10,
                  ...
                  <Grid item xs={12}>
                    <Alert
                      severity="warning"
                      sx={{ bgcolor: "#fff3cd", borderColor: "#6d1313" }}
                    >
                      <strong>Importante:</strong> Los participantes deberán
                      cumplir con al menos el{" "}
                      {curso.porcentaje_asistencia_aprobacion}% de asistencia y
                      obtener una nota mínima de {curso.nota_minima_aprobacion}{" "}
                      para aprobar el curso y recibir su certificado.
                    </Alert>
                  </Grid>
                </Grid>
              </Grid>
          ...
          <DialogActions sx={{ p: 3, bgcolor: "#f5f5f5" }}>
            ...
            <Button
              ...
              startIcon={
                loadingModal ? <CircularProgress size={20} /> : <Save />
              }
              disabled={loadingModal}
              sx={{
                bgcolor: "#6d1313",
